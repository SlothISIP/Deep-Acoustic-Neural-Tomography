...
Laplacian at only $r = 0.19$ (3.5\% variance explained), because random
Fourier features with bandwidth $\sigma = 30$ amplify second derivatives by
$4\pi^2\sigma^2 \approx 35{,}000\times$.
\end{abstract}
...
indicating per-frequency memorization rather than spectral continuity.
...
At $\sigma = 30$\,m$^{-1}$ (required for resolving $k_{\max} = 146$\,rad/m),
this gives $4\pi^2 \cdot 900 \approx 35{,}000\times$ amplification
(Fig.~\ref{fig:helmholtz}b), compounded to ${\sim}55{,}000\times$ by the
8-layer ResNet. High $\sigma$ is needed for forward accuracy, but it makes
...
second derivatives by $\sim$35{,}000$\times$, reducing the neural-physical
Laplacian correlation to $r = 0.19$.
This quantifies the gap between network curvature and physical
Laplacians, with implications for the PINN literature on acoustic
surrogate models.
