...
This simple reformulation increases explained variance from 13\%
(raw pressure) to 89.6\%, enabling a compact MLP to approximate
BEM-quality fields.
...
\item \textbf{Comprehensive evaluation}: cycle-consistency ($r=0.90$),
...
Green's function in 2D. The uniform theory of diffraction~\cite{kouyoumjian1974uniform}
provides the analytical basis for edge scattering in such geometries.
...
reduces error to 2.27\% even \emph{without} Fourier features,
demonstrating that the transfer function---not the encoding---is the key
contribution. Fourier features accelerate convergence (fewer epochs to
reach low error) but are not required for final accuracy.
...
Thirteen of 15 scenes achieve IoU\,$>$\,0.92 (Fig.~\ref{fig:sdf_gallery});
the exceptions are Scene~5 (thin barrier, IoU\,=\,0.88) and
...
mean Chamfer distance 0.063\,m and mean Hausdorff distance 0.456\,m
across the 15 scenes (Fig.~\ref{fig:ablation}).
...
(b)~Inverse: each component contributes to the final IoU of 0.95.}
\label{fig:ablation}
\end{figure}
...
frequency-continuous representation. Interpolation similarly gives 39.62\%
test error---the 60\,Hz spectral gap between training frequencies
exceeds the model's generalization bandwidth.
...
4.47\%, a $>$10$\times$ improvement that holds even without Fourier
feature encoding (2.27\%). The auto-decoder inverse model with Eikonal
regularization achieves 0.95 mean IoU, and cycle-consistency provides
acoustic validation ($r = 0.90$) robust to 10\,dB noise. Frequency
